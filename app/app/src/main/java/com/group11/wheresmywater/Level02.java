...
public class Level02 extends BaseScene implements IOnSceneTouchListener,
	...
	private void createCoEmpty() {
		...
		Vector2[] pos = {new Vector2(150, 410), new Vector2(230, 410),
				new Vector2(310, 410) };
...
/*
	private void excuteCollidesWithCo(int index) {
		// TODO Auto-generated method stub
		Sprite coEmpty = listCoEmpty.get(index);
		int count = coEmpty.getTag() + 1;
		if (count >= 5) {
			this.detachChild(coEmpty);
			listCoEmpty.remove(index);
			_resource.soundDucky.play();
			Sprite co = listCo.get(nCoHaveWater);
			this.detachChild(co);
			co = new Sprite(co.getX(), co.getY(),
					_resource.listcowater_region.get(5), _vbom);
			this.attachChild(co);
			nCoHaveWater++;
		} else {
			this.detachChild(coEmpty);
			coEmpty = new Sprite(coEmpty.getX(), coEmpty.getY(),
					_resource.listcowater_region.get(count), _vbom);
			coEmpty.setTag(count);
			listCoEmpty.remove(index);
			listCoEmpty.add(index, coEmpty);
			this.attachChild(coEmpty);
		}
	}
*/
	 ...
	 *check
	 */
	private boolean check(float x, float y, float _x, float _y){
		...
		if(_x >= x && _x <= x + 10f && y == _y){
			return true;
		}
		if(_y >= y && _y <= y + 10f && x == _x){
			return true;
		}
		if(_x >= x && _x <= x + 10f && y == _y + 10f){
			return true;
		}
		if(_y >= y && _y <= y + 10f && x == _x + 10f){
			return true;
		}
		return isCheck;
	}
