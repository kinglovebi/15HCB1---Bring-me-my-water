 ...
 * The Class Level18.
 */
public class Level18 extends BaseScene implements IOnSceneTouchListener,
        IOnMenuItemClickListener {

    /** The _resource. */
    private Level18Resource _resource;
    ...
    private ArrayList<Sprite> listCoEmpty;

    /** The list ducky. */
    private ArrayList<Sprite> listDucky;

    private ArrayList<Sprite> listCo;
    ...
    public void createScene() {
        ...
        _resource = ResourcesManager.getInstance()._level18Resource;
    ...
    private void createDuckyEmpty() {
        ...
        Vector2[] pos = {new Vector2(630, 425), new Vector2(620, 910),
                new Vector2(290, 630) };
    ...
    private void createCoEmpty() {
        // TODO Auto-generated method stub
        int nCo = 2;
        float top = 10;
        float left = 10;
        listCoEmpty = new ArrayList<Sprite>();
        listCo = new ArrayList<Sprite>();
        Vector2[] pos = {new Vector2(250, 450), new Vector2(330, 450),
                new Vector2(310, 410) };

        for (int i = 0; i < nCo; i++) {
            float topEmpty = pos[i].y;
            float leftEmpty = pos[i].x;
            Sprite coEmpty = new Sprite(leftEmpty, topEmpty,
                    _resource.listcowater_region.get(0), _vbom);
            coEmpty.setTag(0);
            listCoEmpty.add(coEmpty);
            coEmpty.setZIndex(0);
            touch(leftEmpty + 32, topEmpty + 32);
            touch(leftEmpty + 65, topEmpty + 32);

            Sprite co = new Sprite(left + i * 70, top,
                    _resource.listcowater_region.get(0), _vbom);
            listCo.add(co);
            coEmpty.setZIndex(4);
            this.attachChild(coEmpty);
            //this.attachChild(co);
    ...
    private void createSoil() {
        ...
        touch(130, 310); touch(135, 310);
        touch(140, 310); touch(145, 310);touch(150, 310);
        for(int i = 630; i<=780;i+=50){
            touch(540, i);
           // touch(540, i);
        }
    ...
    private void createBody() {
        ...
        BodyProvider.generateBodies("body/body_level18.xml", mPhysicsWorld,
                FIXTURE_DEF);
    }
    ...
    public void disposeScene() {
        // TODO Auto-generated method stub
        stopMusic();
        ResourcesManager.getInstance().unloadLevel18Screen();
        System.out.println("Level18 Dispose");
    }
    ...
    private void updateWater() {
        for (int i = 0; i < listWater.size(); i++) {
            ...
            int k = 1;
            Sprite co = listCoEmpty.get(k);
            if (rect1.collidesWith(co)) {
                this.detachChild(sprite);
                listBody.remove(i);
                listWater.remove(i);
            }
    ...
    private void excuteCollidesWithPipe(int index) {
        ...
        sprite.setPosition(170, 820);
        Rectangle rect = new Rectangle(170 + 42.5f, 820 + 80, 5, 5, _vbom);
    ...
    public void load() {
        // TODO Auto-generated method stub
        ResourcesManager.getInstance().loadLevel18Screen();
    }
}