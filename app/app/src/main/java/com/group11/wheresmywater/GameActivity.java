...
public class GameActivity extends BaseGameActivity implements OnClickListener {
	...
	public final void onClick(final View v) {
		...
		case R.id.btnlevels:
			mEngine.start();
			int ids = Integer.parseInt(mEngine.getScene().toString().split(" ")[0].toString().substring(5));
			if(ids <=10){
				SceneManager.getInstance().loadSelectLevelScene(mEngine);
			}else if (ids <=20){
				SceneManager.getInstance().loadSelectLevelScene1(mEngine);
			} else if(ids <=30){
				SceneManager.getInstance().loadSelectLevelScene2(mEngine);
			}
			dialog.dismiss();
			break;
		default:
