...
public class ScoreScene extends BaseScene implements IOnMenuItemClickListener {
	...
	private void save() {
		...
		String txtname = "wmw.txt";
		int index = Global.IDScene - 1;
		int idscene = Global.IDScene;
		...
		if (Integer.parseInt(s[index].split(",")[2]) < Global.nDuckyHaveWater)
			s[index] = String.valueOf(idscene) + "," + "1"
	...
	public boolean onMenuItemClicked(MenuScene pMenuScene, IMenuItem pMenuItem,
		...
		case LEVEL_MENU: // Huy setup cac man choi duoc xuat hien theo dung quy tac
			if(Global.IDScene <=9){
				...
				isDirect = true;
			}else if(Global.IDScene >= 10 && Global.IDScene <=19){
		...
		case NEXT_LEVEL_MENU:
			if (Global.IDScene < Global.nScene + 20) {
